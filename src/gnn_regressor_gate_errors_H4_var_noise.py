...
def train_and_evaluate(model, train_loader, val_loader, optimizer, epochs, beta, bond_length):
    ...
    for epoch in range(epochs):
        ...
        # save best loss model 
            model_save_path =  'src/models/'+str(bond_length)+'times_gnn_model_cnot_val_huber_loss_ge_tr_gua_h4_3pram_var_noise_level0.pt'
...
def test_model(model, test_data, circ_features, bond_length):
    ...
    model = CombinedModel(num_features=4, num_nodes=8, hidden_dim=64)
    model.load_state_dict(torch.load('src/models/'+str(bond_length)+'times_gnn_model_cnot_val_huber_loss_ge_tr_gua_h4_3pram_var_noise_level0.pt'))
    model.eval()
...
def main(args):
    ...
    df = pd.read_csv('src/tmp/var_noise/'+str(bond_length)+'times_noise_train_data_H4_dep_noise_level0.csv')
    # df = df[:469]
