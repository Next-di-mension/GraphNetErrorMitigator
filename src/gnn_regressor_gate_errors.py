...
def cnot(bond_length):
    with open('src\cnot_connetivity_'+str(bond_length)+'times_tr.txt', 'r') as file:
...
def train_and_evaluate(model, train_loader, val_loader, optimizer, epochs, beta, bond_length):
    ...
    for epoch in range(epochs):
        ...
        # save best loss model 
            model_save_path =  'src/models/'+str(bond_length)+'times_gnn_model_cnot_val_huber_loss_ge_tr_seq.pt'
        ...
        noisy_val = torch.tensor([[-1.38815108140589, 140/264,  124/264, 1]])
...
def test_model(model, test_data, circ_features, bond_length):
    ...
    model.load_state_dict(torch.load('src/models/'+str(bond_length)+'times_gnn_model_cnot_val_huber_loss_ge_tr_seq.pt'))
    model.eval()
...
def main(args):
    ...
    df = pd.read_csv('src/tmp/'+str(bond_length)+'times_noise_data.csv')
    ...
    targets = torch.tensor(df['REM_seq'].values.reshape(-1, 1), dtype=torch.float32)
