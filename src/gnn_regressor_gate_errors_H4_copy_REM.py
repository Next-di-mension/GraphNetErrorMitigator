...
def train_and_evaluate(model, train_loader, val_loader, optimizer, epochs, beta, bond_length):
    ...
    for epoch in range(epochs):
        ...
        if train_loss < best_train_loss:
            ...
            model_save_path =  'src/models/'+str(bond_length)+'times_gnn_model_cnot_val_huber_loss_ge_tr_mel_bh_3pram_rem_train_loss.pt'
        ...
        # save best loss model 
            model_save_path =  'src/models/'+str(bond_length)+'times_gnn_model_cnot_val_huber_loss_ge_tr_Mel_BH_3pram_rem.pt'
            torch.save(model.state_dict(), model_save_path)
...
def test_model(model, test_data, circ_features, bond_length):
    ...
    model = CombinedModel(num_features=4, num_nodes=14, hidden_dim=64)
    model.load_state_dict(torch.load('src/models/'+str(bond_length)+'times_gnn_model_cnot_val_huber_loss_ge_tr_Mel_BH_3pram_rem.pt'))
    model.eval()
...
def test_best_train_model(model, test_data, circ_features, bond_length):
        ...
        """Tests the model on new data."""
        model = CombinedModel(num_features=4, num_nodes=14, hidden_dim=64)
        model.load_state_dict(torch.load('src/models/'+str(bond_length)+'times_gnn_model_cnot_val_huber_loss_ge_tr_mel_bh_3pram_rem_train_loss.pt'))
        model.eval()
...
def main(args):
    ...
    df = pd.read_csv('src/tmp/'+str(bond_length)+'times_noise_train_data_Melbourne_BH.csv')
    # df = df[:469]
    ...
    test_data_path = data_path + str(bond_length) + 'times_noise_test_data_Melbourne_' + molecule + '.csv'
    test_data = pd.read_csv(test_data_path)
    test_cnot_col = test_data['cnot_con']
    test_point = torch.tensor(test_data[['Noisy_val_approx', 'two_qc_ratio', 'single_qc_ratio', 'param_ratio']].values, dtype=torch.float32)
    test_graph_data = create_graph_data(num_nodes, 0, test_cnot_col)
