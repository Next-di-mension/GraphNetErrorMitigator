...
def initialize_estimators(shots):
    ...
    # create custom noise model with depolarizing noise
    noise_model = NoiseModel()
    p1Q = 0.0005 
    p2Q = 0.005 # level 0 = 0.00001
    noise_model.add_all_qubit_quantum_error(depolarizing_error(p1Q, 1), ['u1', 'u2', 'u3'])
    noise_model.add_all_qubit_quantum_error(depolarizing_error(p2Q, 2), ['cx'])


    noisy_estimator = AerEstimator(
        backend_options={"method": "statevector", "noise_model": noise_model},
...
def gate_count(qc):
    ...
    tr_qc = transpile(qc, backend=backend,optimization_level=3,seed_transpiler=SEED)
...
def main(args):
    ...
    # Initialize ansatz
    key = molecule + '_'+ str(bond_length) + 'times_level' + str(level)
    ansatz = Ansatz[key]
    ...
    # Save results to CSV
    path = data_path+str(bond_length)+'times_noise_train_data_' + molecule + '_'  + 'dep_noise_level' + str(level) + '.csv'
