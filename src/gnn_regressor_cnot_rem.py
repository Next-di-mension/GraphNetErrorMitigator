...
def cnot():
    with open('src\cnot_connetivity_2times.txt', 'r') as file:
...
class CombinedModel(nn.Module):
    ...
    def forward(self, x, edge_index, edge_weight, noisy_value):
        ...
        # dropout
        # combined = F.dropout(combined, p = 0.1)
...
def train_and_evaluate(model, train_loader, val_loader, optimizer, epochs):
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            ...
            # huber loss
            # loss = F.smooth_l1_loss(output, target, beta=0.1) # beta = 0.4 for 1times
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                # val_loss += F.smooth_l1_loss(output, target, beta=0.1).item()
        ...
        # save best loss model 
            model_save_path =  'src/models/2times_gnn_model_cnot_val_huber_loss_rem.pt'
        ...
        noisy_val = torch.tensor([[-1.315982022, 80/224,  60/224, 1]])
        test_graph_data = create_graph_data(-1)  # Example test data
...
def test_model(model, test_data, circ_features):
    ...
    model.load_state_dict(torch.load('src/models/2times_gnn_model_cnot_val_huber_loss_rem.pt'))
    model.eval()
...
def main(args):
    ...
    targets = torch.tensor(df['REM_val_improved'].values.reshape(-1, 1), dtype=torch.float32)
    operator_col = df['Operator']

    # Prepare dataset
    dataset = [(create_graph_data(i), inputs[i], targets[i]) for i in range(len(operator_col))]
