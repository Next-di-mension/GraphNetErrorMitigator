...
def cnot():
    with open('src\cnot_connetivity_25times.txt', 'r') as file:
...
def train_and_evaluate(model, train_loader, val_loader, optimizer, epochs):
    ...
    for epoch in range(epochs):
        ...
        # save best loss model 
            model_save_path =  'src/models/25times_gnn_model_cnot_val_huber_loss.pt'
        ...
        noisy_val = torch.tensor([[-1.335096611, 80/224,  60/224, 1]])
...
def test_model(model, test_data, circ_features):
    ...
    model.load_state_dict(torch.load('src/models/25times_gnn_model_cnot_val_huber_loss.pt'))
    model.eval()
