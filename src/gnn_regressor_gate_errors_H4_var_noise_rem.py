    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for batch in val_loader:
                target = batch[2].reshape(1, -1)
                output = model(batch[0].x, batch[0].edge_index, batch[0].edge_attr, batch[1])
                # print('lss', (output-target).item()**2)
        ...
        # save best loss model 
            model_save_path =  'src/models/'+str(bond_length)+'times_gnn_model_cnot_val_huber_loss_ge_tr_gua_h4_3pram_rem_var_noise_level'+str(level)+'.pt'
...
def test_model(model, test_data, circ_features, bond_length):
    ...
    model = CombinedModel(num_features=6, num_nodes=8, hidden_dim=64)
    model.load_state_dict(torch.load('src/models/'+str(bond_length)+'times_gnn_model_cnot_val_huber_loss_ge_tr_gua_h4_3pram_rem_var_noise_level'+str(level)+'.pt'))
    model.eval()
