...
def train_and_evaluate(model, train_loader, val_loader, optimizer, epochs, beta, bond_length):
    ...
    for epoch in range(epochs):
        ...
        # save best loss model 
            model_save_path =  'src/models/'+str(bond_length)+'times_regressor_' + 'dep_noise_level'+str(level)+'.pt'
        ...
        test_point = torch.tensor(test_data[['Noisy_val_approx']].values, dtype=torch.float32)
...
def test_model(model, test_data, bond_length):
    ...
    model = Regressor(num_features=1, hidden_dim=64)
    model.load_state_dict(torch.load('src/models/'+str(bond_length)+'times_regressor_' + 'dep_noise_level'+str(level)+'.pt'))
    model.eval()
...
def main(args):
    ...
    inputs = torch.tensor(df[['Noisy_val_approx']].values, dtype=torch.float32)
