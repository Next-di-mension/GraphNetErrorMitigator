...
def train_and_evaluate(model, train_loader, val_loader, optimizer, epochs, beta, bond_length):
    ...
    for epoch in range(epochs):
        ...
        if train_loss < best_train_loss:
            ...
            model_save_path =  'src/models/'+str(bond_length)+'times_gnn_model_cnot_val_huber_loss_ge_tr_mel_bh_3pram_train_loss.pt'
        ...
        # save best loss model 
            model_save_path =  'src/models/'+str(bond_length)+'times_gnn_model_cnot_val_huber_loss_ge_tr_Mel_BH_3pram.pt'
            torch.save(model.state_dict(), model_save_path)
...
def test_model(model, test_data, circ_features, bond_length):
    ...
    model.load_state_dict(torch.load('src/models/'+str(bond_length)+'times_gnn_model_cnot_val_huber_loss_ge_tr_Mel_BH_3pram.pt'))
    model.eval()
...
def test_best_train_model(model, test_data, circ_features, bond_length):
        ...
        """Tests the model on new data."""
        model = CombinedModel(num_features=4, num_nodes=14, hidden_dim=64)
        model.load_state_dict(torch.load('src/models/'+str(bond_length)+'times_gnn_model_cnot_val_huber_loss_ge_tr_mel_bh_3pram_train_loss.pt'))
        model.eval()
